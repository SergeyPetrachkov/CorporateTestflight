...
@CacheActor
@Suite("Global Actor Image Cache Tests")
struct GlobalActorImageLoaderTests {
	...
	struct Environment {
		...
		func makeSUT() -> GlobalActorImageCache {
			GlobalActorImageCache(apiService: apiService)
		}
	}
	...
	func returnsImageWhenImageExists() async throws {
		...
		let _ = try await sut.load(url: env.url)
	...
	func throwsErrorWhenImageDataIsInvalid() async throws {
		...
		await #expect(throws: GlobalActorImageCache.ImageCacheError.failedDownloadingImage(env.url)) {
			_ = try await sut.load(url: env.url)
		}
	}
